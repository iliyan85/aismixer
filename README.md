...
- **`nmea_sproxy`** is the station-side UDPSEC proxy. One process forwards one
  local UDP or physical serial input to one AISMixer UDPSEC input.
...
through the same UDPSEC relation. This does not add plain UDP output.
...
- **`nmea_sproxy`** е UDPSEC проксито при станцията. Един процес препраща един
  локален UDP или физически serial вход към един UDPSEC вход на AISMixer.
...
```text
един локален вход (UDP или serial) → един криптиран UDPSEC вход на AISMixer
```
